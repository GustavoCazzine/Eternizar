...
if (returnIdx === -1) { console.log('RETURN NOT FOUND'); process.exit(1); }

// Keep everything before the return
const logic = c.slice(0, returnIdx);
...
const render = `
 ...
 <div ref={containerRef} className="text-white relative"
   ...
   <section className="min-h-[100dvh] flex flex-col items-center justify-center relative overflow-hidden">
     {fotoCapa && (
       <div className="absolute inset-0 pointer-events-none">
         ...
         <img src={fotoCapa} alt="" className="w-full h-full object-cover opacity-50"
           style={{ objectPosition: 'center 30%' }} />
         <div className="absolute inset-0"
           style={{ background: 'linear-gradient(to bottom, transparent 0%, rgba(0,0,0,0.6) 50%, #000 100%)' }} />
       </div>
     )}
     ...
     <div className="relative z-10 text-center px-6 max-w-2xl">
       ...
       <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
         ...
         className="text-[10px] sm:text-xs uppercase tracking-[0.3em] mb-4 sm:mb-6"
         style={{ color: cor }}>
       ...
       {pagina.subtitulo && (
         <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
           ...
           className="text-base sm:text-lg mt-4 sm:mt-6 nome-capitalize"
   ...
   {pagina.tipo === 'casal' && pagina.dados_casal?.dataInicio && (
     <section className="min-h-[100dvh] flex items-center justify-center px-6 relative">
       ...
       <Secao className="text-center max-w-xl w-full">
         <p className="text-[10px] sm:text-xs uppercase tracking-[0.3em] mb-8 sm:mb-12" style={{ color: cor }}>
         ...
         {pagina.musica_dados?.previewUrl && (
           <Secao delay={0.3} className="mt-12 sm:mt-16">
             <div className="flex items-center gap-4 max-w-sm mx-auto">
               <button onClick={() => {
                 if (!audioRef.current) {
                   audioRef.current = new Audio(pagina.musica_dados!.previewUrl!)
   ...
   {fotosNormalizadas.length > 0 && (
     <section className="min-h-[80dvh] flex items-center justify-center px-6 py-20">
       <Secao className="w-full max-w-4xl">
         <p className="text-[10px] sm:text-xs uppercase tracking-[0.3em] mb-8 sm:mb-12 text-center" style={{ color: cor }}>
           Nossos momentos
         </p>
         ...
         {fotosNormalizadas.length > 6 && (
           <button onClick={() => { setStoryInicial(0); setStoriesAberto(true) }}
             className="mt-6 mx-auto flex items-center gap-2 text-sm"
             style={{ color: cor }}>
             <Images className="w-4 h-4" />
             Ver todas as {fotosNormalizadas.length} fotos
           </button>
         )}
       </Secao>
     </section>
   ...
   {storiesAberto && (
     <StoriesViewer fotos={fotosNormalizadas} startIndex={storyInicial}
       onClose={() => setStoriesAberto(false)} cor={cor} />
   )}
   ...
   {pagina.linha_do_tempo && pagina.linha_do_tempo.length > 0 && (
     <section className="py-16 sm:py-24 px-6">
       <Secao className="max-w-2xl mx-auto">
         <p className="text-[10px] sm:text-xs uppercase tracking-[0.3em] mb-12 sm:mb-16 text-center" style={{ color: cor }}>
           Nossa timeline
         </p>
         ...
         <div className="space-y-16 sm:space-y-24">
           {pagina.linha_do_tempo.map((ev, i) => (
             <motion.div key={i}
               ...
               {/* Title */}
               <h3 className="text-xl sm:text-2xl md:text-3xl font-black tracking-tight mb-3 break-words">
                 {ev.titulo}
               </h3>

               {ev.descricao && (
                 <p className="text-sm sm:text-base text-white/60 leading-relaxed max-w-lg mx-auto break-words">
                   {ev.descricao}
                 </p>
               )}
   ...
   <section className="min-h-[80dvh] flex items-center justify-center px-6 py-20">
     <Secao className="text-center max-w-2xl">
       ...
       <p className="text-lg sm:text-xl md:text-2xl leading-relaxed text-white/85 whitespace-pre-wrap break-words">
         {pagina.mensagem}
       </p>
       <div className="w-12 h-px mx-auto mt-8 sm:mt-12" style={{ background: cor }} />
     </Secao>
         ...
         <div className="space-y-3">
           {(pagina.bucket_list as any[]).map((item: any, i: number) => (
             <motion.div key={i}
               ...
               <span className={\`text-sm sm:text-base \${item.feito ? 'line-through text-white/40' : 'text-white/80'}\`}>
                 {item.texto}
               </span>
             </motion.div>
           ))}
   ...
   <section className="py-16 sm:py-20 px-6">
     <Secao className="max-w-lg mx-auto">
       ...
       <div className="space-y-3 mb-10">
         <input value={guestNome} onChange={e => setGuestNome(e.target.value)}
         ...
         <button onClick={enviarGuestbook}
           disabled={guestEnviando || !guestNome.trim() || !guestMsg.trim()}
           className="w-full py-3 rounded-full text-sm font-bold text-white transition hover:opacity-90 disabled:opacity-30"
           ...
           {guestMsgs.filter(m => m.aprovado !== false || ehDono).map(msg => (
             <motion.div key={msg.id}
               ...
               {ehDono && msg.aprovado === false && (
                 <button onClick={() => aprovarMsg(msg.id)}
                   className="text-[10px] mt-2 px-3 py-1 rounded-full" style={{ background: \`\${cor}20\`, color: cor }}>
                   Aprovar
                 </button>
               )}
             </motion.div>
   ...
   <section className="py-20 sm:py-28 text-center px-6">
     <Secao className="max-w-md mx-auto">
       ...
       <p className="text-xl sm:text-2xl font-black tracking-tight mb-4" style={{ color: 'rgba(255,255,255,0.5)' }}>
         A historia continua...
       </p>
...
console.log('Render rewritten. Size:', Math.round(c.length / 1024) + 'KB');
